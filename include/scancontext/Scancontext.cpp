...
void SCManager::makeAndSaveScancontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down, const int currentFloor )
{
    Eigen::MatrixXd sc = makeScancontext(_scan_down); // v1 
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext( sc );
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext( sc );
    ...
    polarcontexts_.push_back( sc ); 
    polarcontexts_map_[currentFloor].push_back(sc);
    polarcontext_invkeys_.push_back( ringkey );
    polarcontext_vkeys_.push_back( sectorkey );
    polarcontext_invkeys_mat_.push_back( polarcontext_invkey_vec );
    absoluteNodeIDVec[currentFloor].push_back(polarcontext_invkeys_mat_.size());
    polarcontext_invkeys_mat_map_[currentFloor].push_back( polarcontext_invkey_vec );
} // SCManager::makeAndSaveScancontextAndKeys
...
std::pair<int, float> SCManager::detectLoopClosureID ( const int currentFloor )
{
    int loop_id { -1 }; // init with -1, -1 means no loop (== LeGO-LOAM's variable "closestHistoryFrameID")
     ...
     * step 1: candidates from ringkey tree_
     */
    if( (int)polarcontext_invkeys_mat_map_[currentFloor].size() < NUM_EXCLUDE_RECENT + 1)
    {
        std::pair<int, float> result {loop_id, 0.0};
        return result; // Early return 
    }
    ...
    // tree_ reconstruction (not mandatory to make everytime)
    // if( tree_making_period_conter % TREE_MAKING_PERIOD_ == 0) // to save computation cost
    if(floor != currentFloor || !isTreeReconstructed)
    {
        if(floor != currentFloor) floor = currentFloor;
        if(!isTreeReconstructed) isTreeReconstructed = true;
        TicTocV2 t_tree_construction;

        polarcontext_invkeys_to_search_.clear();
        polarcontext_invkeys_to_search_.assign( polarcontext_invkeys_mat_map_[currentFloor].begin(), polarcontext_invkeys_mat_map_[currentFloor].end() - NUM_EXCLUDE_RECENT ) ;
    ...
    for ( int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ )
    {
        if(candidate_iter_idx < candidate_indexes.size()) {
            MatrixXd polarcontext_candidate = polarcontexts_map_[currentFloor][ candidate_indexes[candidate_iter_idx] ];
            std::pair<double, int> sc_dist_result = distanceBtnScanContext( curr_desc, polarcontext_candidate ); 
    ...
    {
        // loop_id = nn_idx; 
        loop_id = absoluteNodeIDVec[currentFloor].at(nn_idx)+1;
    
        // std::cout.precision(3); 
        cout << "[Loop found] Nearest distance: " << min_dist << " btn " << polarcontexts_.size()-1 << " and " << loop_id << "." << endl;
        // cout << "[Loop found] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl;
    }
    else
    {
        std::cout.precision(3); 
        // cout << currentFloor << " " << "[Not loop] Nearest distance: " << min_dist << " btn " << polarcontexts_map_[currentFloor].size()-1 << " and " << nn_idx << "." << endl;
        // cout << "[Not loop] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl;
    }
