...
using KeyMatMap = std::map<int, KeyMat>; // map the KeyMat with floor
...
public: 
    ...
    void makeAndSaveScancontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down , const int currentFloor);
    std::pair<int, float> detectLoopClosureID( void ); // int: nearest node index, float: relative yaw  
    std::pair<int, float> detectLoopClosureID( const int currentFloor ); // int: nearest node index, float: relative yaw  
...
public:
    ...
    std::map<int, std::vector<Eigen::MatrixXd>> polarcontexts_map_;

    KeyMat polarcontext_invkeys_mat_;
    KeyMatMap polarcontext_invkeys_mat_map_;
    ...
    int floor = 0;
    bool isTreeReconstructed = false;
    std::map<int, std::vector<int>> absoluteNodeIDVec; // floor, node id
